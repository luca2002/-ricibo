...
$res_get_area = mysqli_query($db,$qry_get_area);
...
$area = mysqli_fetch_assoc($res_get_area)['ID_AREA'];
...
$qry_get_assocs = "SELECT * FROM TB_NEG_ASS WHERE FLAG_AREA_NEG_ASS = 'A'
	AND TB_NEG_ASS.ID_AREA = '$area'";
$res_get_assocs = mysqli_query($db,$qry_get_assocs);
if(!$res_get_assocs){
	//if($doDebug){
		echo("Error: " . mysqli_error($db));
	//}
...
$assocs = array();
while($row_asscs = mysqli_fetch_array($res_get_assocs)){
	array_push($assocs, $row_asscs);
}

$rvl = array();
while($row_offer = mysqli_fetch_array($res_get_offers)){
