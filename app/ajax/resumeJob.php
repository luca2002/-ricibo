...
$res_find_0 = mysql_query($qry_find_0, $db);

if($res_find_0){
	if(mysql_num_rows($res_find_0) > 0){
		...
		$res_find_1 = mysql_query($qry_find_1, $db);
		if($res_find_1){
			if(mysql_num_rows($res_find_1) > 0){
...
$res_get_offers = mysql_query($qry_get_offers, $db);
if(!$res_get_offers){
	//if($doDebug){
		echo("Error: " . mysql_error($db));
	//}
...
if(mysql_num_rows($res_get_offers) < 1){
	mysql_close();
	die("Not found");
}

$row = mysql_fetch_array($res_get_offers);
