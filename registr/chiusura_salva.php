...
<html>
    ...
    <?php include "../menu.php";
    	...
    	if ($codUscita == (1 || 2)) {
				...
				if (isset($DEL_DATA)) {
					$sql = "DELETE FROM TB_CHIUSURA where ID_NEG_ASS = $ID_NEG_ASS AND GIORNO_CHIUSURA = '$DEL_DATA';";
	        ...
	        $ID_AREA = 0;
	    		...
	    		$GIORNO_SETTIMANA = htmlspecialchars($_POST['DATA']);
			    ...
			    if($codUscita == 0) {
			      $sql = "";
			    	  $sql1 = "INSERT INTO TB_CHIUSURA(ID_AREA, ID_NEG_ASS, GIORNO_CHIUSURA, ORA1_INIZIO, ORA1_FINE, ORA2_INIZIO, ORA2_FINE)";
			    	  $sql2 = "VALUES($ID_AREA, $ID_NEG_ASS, '$GIORNO_SETTIMANA', '$ORARIO_APERTURA1', '$ORARIO_CHIUSURA1', '$ORARIO_APERTURA2', '$ORARIO_CHIUSURA2');";
			    	  $sql = $sql1 . $sql2;
			    	  echo("</BR>sql>$sql<");
			    	  $ris = mysql_query($sql, $db);
			    	  if(!$ris) {
		    ...
		    //header("location: mess_registr.php");	// CASO DI PRIMA REGISTRAZIONE
		    //}
		    //}
	     elseif ($codUscita == -1) {
		   	 echo ("</br>Errore: orario non corretto nel giorno " . $GIORNO_ERRORE . ".</BR>Dati non salvati.");
		   	 $sql = "DELETE FROM TB_CHIUSURA WHERE ID_NEG_ASS = $ID_NEG_ASS;";
		   	 $ris = mysql_query($sql, $db);
		   	 mysql_close($db);
		   	 if (!$ris) {
